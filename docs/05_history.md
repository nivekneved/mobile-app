...
- **Build & Submission Configurations**:
  ...
  - [eas.json](file:///c:/Users/deven/Desktop/Travel%20Lounge%202026/apps/mobile-app/eas.json): Added `EXPO_PUBLIC_API_URL` to `production` environment profile, set iOS build image to `"latest"` (Xcode 17+) to meet Apple's newer App Store requirements, and configured the submission profile with `ascAppId` (`"6785522609"`) and `appleId` (`"devenpawaray@gmail.com"`).
